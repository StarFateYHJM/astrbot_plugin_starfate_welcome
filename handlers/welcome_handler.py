...
body {{{{
    ...
    zoom: {w.get("css_zoom", 2)};
    background: {w.get("background_color", "#1A1A2E")};
    position: relative;
    font-size: {w.get("base_font_size", "16px")};
}}}}
.bg-layer {{{{
    position: absolute;
    top: 0; left: 0;
...
.welcome-container {{{{
    position: relative;
    padding: {w.get("padding_body", "40px 50px")};
    color: {w.get("text_color", "#FFF")};
    z-index: 2;
}}}}
.content h1 {{{{
    font-size: {w.get("h1_font_size", "2.5em")};
    border-bottom: 2px solid {w.get("border_color", "#333")};
    margin-bottom: 20px;
    padding-bottom: 15px;
}}}}
.content h2 {{{{
    font-size: {w.get("h2_font_size", "2em")};
    margin: 30px 0 15px;
}}}}
.content h3 {{{{
    font-size: {w.get("h3_font_size", "1.5em")};
    margin: 25px 0 10px;
}}}}
.content p {{{{
    margin-bottom: 15px;
...
.content a {{{{
    color: {w.get("link_color", "#0DF")};
    text-decoration: none;
}}}}
.content code {{{{
    background: {w.get("code_bg_color", "#2D2D2D")};
    color: {w.get("code_text_color", "#E6E6E6")};
    padding: 2px 6px;
    border-radius: 4px;
}}}}
.content pre {{{{
...
.content th, .content td {{{{
    border: 1px solid {w.get("border_color", "#333")};
    padding: 10px 15px;
}}}}
.content hr {{{{
    border: none;
    border-top: 2px solid {w.get("border_color", "#333")};
    margin: 30px 0;
}}}}
</style></head>
