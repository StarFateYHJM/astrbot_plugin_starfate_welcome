...
class StarFateWelcomePlugin(Star):
    ...
    def resolve_background(self, user_input: str) -> str:
        ...
        if local_path.exists():
            try:
                ...
                self._log(f"本地背景图: {user_input}", "debug")
    ...
    @filter.on_message()
    async def on_welcome(self, event: AstrMessageEvent):
        """监听消息，筛选入群事件"""
        # 检查是否为入群事件
        msg_obj = event.message_obj
        if not hasattr(msg_obj, 'type'):
            return
        if msg_obj.type not in ("GroupMemberIncrease", "group_member_increase"):
            return
    ...
    async def cmd_test(self, event: AstrMessageEvent, welcome_id: str = None):
        ...
        self._log(f"测试: {welcome.get('welcome_id')}", "debug")
        try:
            html = self.handler.render(welcome, event)
            url = await self.html_render(html, {"full_page": True})
            yield event.image_result(url)
